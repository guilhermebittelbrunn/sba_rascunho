export default function Plans({plans}){
    ...
    return(
        <>
            <section className="bg-gray-100 w-full flex">
                <div className="max-w-screen-lg flex-col m-auto justify-center py-24 flex gap-12 items-center">
        ...
        <>
            <div className="main bg-white rounded-lg shadow-2xl px-4 py-6 w-60 max-lg:w-80">
                <div className="card-header200 gap-1 flex flex-col items-start">
                    ...
                    <span className="text-blue-500 font-black text-xs p-1 border-2 border-blue-500 rounded-md">Pagamento único disponível</span>
                    <h2 className="font-bold text-2xl">{plan.tittle}</h2>
                    <p>{plan.price_description}</p>
                    <p>{plan.num_accounts}</p>
                </div>
                <div className="line w-full h-0.5 bg-gray-300 my-2"/>
                <div className="card-main">
                    {plan.benefits.map((benefit, key)=>{
                        return <p className={`w-full text-base ${key}`}>{benefit}</p>
                    })}
                </div>
                <div className="card-footer text-center mt-48 max-lg:mt-8">
