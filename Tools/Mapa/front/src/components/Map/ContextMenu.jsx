    ...
    return(
            ...
            >
                ...
                <div>
                            ...
                            <RightOutlined className="text-sm"/>
                             <ul id="sub-menu-mun" className="absolute w-[240px] hidden p-3 bg-white shadow-xl top-[-35px] rounded-xl flex-col">
                                <li className="flex justify-between items-center hover:bg-gray-100 py-1 hover:cursor-pointer">
                                    ...
                                    <p className="text-sm">{properties.NM_MUN && String(properties.NM_MUN).substring(0,23)}{properties.NM_MUN && properties?.NM_MUN.length > 24 && '...'}</p>
                                </li>
                                <li className="flex justify-between items-center hover:bg-gray-100 py-1 hover:cursor-pointer">
                    ...
                    {contextMenu.layers.length > 0 && contextMenu.layers.map((layer, k)=>{
                        if(layer.rc && layer.properties.NUMERO_PEDIDO){
                            return (
                                    <li key={k} className="relative flex items-center justify-between hover:bg-gray-100 px-2 py-1" onMouseOut={()=>{hiddenSubMenu(`sub-menu-sales-rep-${layer.rc}`)}} onMouseOver={()=>{showSubMenu(`sub-menu-sales-rep-${layer.rc}`)}}>
                                        <div>
                                            <InfoCircleOutlined />
                                            <span className="ml-2 text-base">Venda {layer.rc}</span>
                                        </div>
                                        <RightOutlined className="text-sm"/>
                                        <ul id={`sub-menu-sales-rep-${layer.rc}`} className="absolute w-[240px] hidden p-3 bg-white shadow-xl top-[-35px] rounded-xl flex-col">
                                            <li className="flex justify-between items-center hover:bg-gray-100 py-1 hover:cursor-pointer">
                                            ...
                                            <details>
                                                ...
                                                <li className="px-1 w-full text-sm flex justify-between"> 
                                                    ...
                                                    <p className="ml-4 text-right text-sm">{String(layer.properties.NOME_CLIENTE).substring(0,19)}{layer.properties.NOME_CLIENTE.length > 21 && '...'}</p>
                                                </li>
                                                ...
                                                <li className="px-1 w-full text-sm flex justify-between"> 
                                                    ...
                                                    <p className="ml-4 text-right">{moment(layer.properties.ULTIMA_VENDA).format('DD/MM/yyyy')}</p>
                                                </li>
                                            </details>
                                        </ul>
                                    </li>
