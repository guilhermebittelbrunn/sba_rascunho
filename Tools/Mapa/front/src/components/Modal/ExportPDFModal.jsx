...
export default function ExportPDFModal({ handleCancel, isModalOpen }) {
  ...
  const { map, rc, layers, createFeatureStyle, settings, url } =
    useContext(MapaContext);

  const sendForm = (data) => {
    setIsLoading(true);
    const dim =
      data.orientation === "Paisagem"
        ? dims[data.paperSize]
        : [...dims[data.paperSize]].reverse();
    const width = Math.round((dim[0] * data.dpiValue) / 25.4);
    const height = Math.round((dim[1] * data.dpiValue) / 25.4);
    const size = map.getSize();
    const viewResolution = map.getView().getResolution();
    const pdf = new jsPDF(
      data.orientation === "Paisagem" ? "landscape" : "portrait",
      "mm",
      dim
    );
    const baseLayer =
      layers[layers.findIndex((layer) => layer.value === "baseLayer")]
        .properties;
    ...
    map.once("rendercomplete", async function () {
      try {
        ...
        if (status > 0) {
          ...
          const canvasWidth =
            ((dim[1] * (table.offsetHeight * 100)) / 2480 / 100) * 2;
          const canvasHeight =
            ((dim[0] * (table.offsetWidth * 100)) / 3508 / 100) * 2;
          const padding = 2;
        ...
        pdf.output("save", {
          filename: `map_${rc}_${data.paperSize}_${data.dpiValue}`,
        });
        map.setSize(size);
  ...
  return (
    ...
    >
      <form
        className="flex flex-col gap-2 justify-center ml-2"
      ...
      >
        ...
        <div className="flex  gap-6 mb-12">
          <div className="w-2/4">
            <div className="flex items-center justify-between gap-2 mt-2">
              <h3 className="font-semibold">Sobrepor</h3>
